...
const DoctorDBS = () => {
  ...
  useEffect(() => {
    const filterConditions = { name: filterText, phone_number: filterText }
    const filteredData = categories.filter((item) =>
      Object.entries(filterConditions).every(([key, value]) => item[key] === filterConditions[key]),
    )
    setFilterItem(filteredData)
    if (filterText === '') {
      setFilterItem(categories)
    }
  }, [filterText])
